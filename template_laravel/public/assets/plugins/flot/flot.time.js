...
(function (root, factory) {
  ...
  function init(plot) {
    plot.hooks.processOptions.push(function (plot, options) {
      var axes = plot.getAxes(), axisName, axis;

      for (axisName in axes) {
        if (!axes.hasOwnProperty(axisName)) continue;

        axis = axes[axisName];

        if (axis.options.mode == "time") {
          ...
          axis.tickFormatter = function (v, axis) {
            ...
            return formatDate(d, fmt, opts.monthNames, opts.dayNames);
          };
        }
      }
    });
